...
function CartItems(){
  const {total,cart,removeFromCart} = useValue();
  const finaltotal = total < 500 && total !== 0 ? total + 75 : total;
    ...
    <div className="flex flex-col md:flex-row gap-6">
              ...
              <tr key={item.id}>
                <td className="py-4">
                  <div className="flex items-center object-cover  h-22 w-24">
                    <img className=" w-full h-full mr-10 " src={item.img} alt="Product image" />
                ...
                <td className="py-4 text-lg">&#8377;{item.qty * item.price}</td>
                <td className="py-4 text-lg">
                <MdDelete onClick={()=>removeFromCart(item.id)} className="size-8 cursor-pointer hover:text-red-700" />
                </td>
      ...
      <div className="md:w-1/4">
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="flex justify-between mb-2 text-lg ">
            <span>Shipping</span>
            <span>&#8377;{total < 500 && total != 0 ? 75 : 0.00}</span>
          </div>
          <hr className="my-2" />
          <div className="flex justify-between mb-2 text-lg ">
